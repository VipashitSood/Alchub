...
namespace Nop.Plugin.Api.Models.ShoppingCartsParameters
{
    ...
    // JsonProperty is used only for swagger
    [ModelBinder(typeof(ParametersModelBinder<WishlistItemsParametersModel>))]
    public class WishlistItemsParametersModel
    {
        public WishlistItemsParametersModel()
        {
            Limit = Constants.Configurations.DEFAULT_LIMIT;
            Page = Constants.Configurations.DEFAULT_PAGE_VALUE;
        }
        ...
        /// <summary>
        /// if null, get all wishlist cart items for all customers
        /// </summary>
        [JsonProperty("customer_id", Required = Required.AllowNull)]
        public int? CustomerId { get; set; }
    }
