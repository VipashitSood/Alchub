...
namespace Nop.Plugin.Api.Controllers
{
    public class OrdersController : BaseApiController
    {
        ...
        public async Task<BaseResponseModel> GetOrders([FromQuery] OrdersParametersModel parameters)
        {
            ...
            try
            {
                ...
                //get orders after filter
                var orders = await _orderApiService.GetOrders(parameters.Ids, parameters.CreatedAtMin,
                ...
                var ordersRootObject = new OrdersListObject
                {
                    Orders = ordersAsDtos,
                    PageIndex = orders.PageIndex + 1,
                    PageSize = orders.PageSize,
                    TotalRecords = orders.TotalCount
                };

                return SuccessResponse(await _localizationService.GetResourceAsync("Nop.Api.Order.List"), ordersRootObject);
            }
            catch (Exception ex)
            {
        ...
        [GetRequestsErrorInterceptorActionFilter]
        public async Task<BaseResponseModel> GetOrderById([FromRoute] int id, [FromQuery] string fields = "")
        {
            if (id <= 0)
            ...
            try
            {
                ...
                //success
                return SuccessResponse(await _localizationService.GetResourceAsync("Nop.Api.Order"), ordersRootObject);
            }
            catch (Exception ex)
            {
                return ErrorResponse(ex.Message, HttpStatusCode.BadRequest);
            ...
            Delta<OrderDto> orderDelta)
        {
            ...
            var customer = await _customerService.GetCustomerByIdAsync(orderDelta.Dto.CustomerId.Value);
            ...
            await _customerService.UpdateCustomerAsync(customer); // update billing and shipping addresses
            ...
            await _customerActivityService.InsertActivityAsync("AddNewOrder", await _localizationService.GetResourceAsync("ActivityLog.AddNewOrder"), newOrder);
            ...
            Delta<OrderDto> orderDelta)
        {
            ...
            var customer = await _customerService.GetCustomerByIdAsync(currentOrder.CustomerId);
            ...
            await _customerActivityService.InsertActivityAsync("UpdateOrder", await _localizationService.GetResourceAsync("ActivityLog.UpdateOrder"), currentOrder);
        ...
        public async Task<IActionResult> DeleteOrder([FromRoute] int id)
        {
            ...
            //activity log
            await _customerActivityService.InsertActivityAsync("DeleteOrder", await _localizationService.GetResourceAsync("ActivityLog.DeleteOrder"), orderToDelete);
        ...
        #region Reward points list

        /// <summary>
        ///     Get reward point list
        /// </summary>
        /// <response code="200">OK</response>
        /// <response code="401">Unauthorized</response>
        [HttpGet]
        [Route("/api/customers/reward_points_list", Name = "GetRewardPointsList")]
        [ProducesResponseType(typeof(CustomerRewardPointsDto), (int)HttpStatusCode.OK)]
        [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType(typeof(ErrorsRootObject), (int)HttpStatusCode.BadRequest)]
        [GetRequestsErrorInterceptorActionFilter]
        public async Task<BaseResponseModel> CustomerRewardPoints([FromQuery] CustomerRewardPointsModel customerRewardPointsModel)
        {
            if (customerRewardPointsModel.PageSize < Constants.Configurations.MIN_LIMIT || customerRewardPointsModel.PageSize > Constants.Configurations.MAX_LIMIT)
                return ErrorResponse("invalid limit parameter", HttpStatusCode.BadRequest);

            if (customerRewardPointsModel.PageSize < Constants.Configurations.DEFAULT_PAGE_VALUE)
                return ErrorResponse("invalid page parameter", HttpStatusCode.BadRequest);

            var customer = await _customerService.GetCustomerByIdAsync(customerRewardPointsModel.UserId);
            if (customer is null)
                return ErrorResponse("Customer not found", HttpStatusCode.BadRequest);
            try
            {
                if (!await _customerService.IsRegisteredAsync(customer))
                    return ErrorResponse(await _localizationService.GetResourceAsync("CustomerNotRegistered"));

                if (!_rewardPointsSettings.Enabled)
                    return ErrorResponse(await _localizationService.GetResourceAsync("CustomerNotEnabled"));

                var store = await _storeContext.GetCurrentStoreAsync();

                var model = await _dtoHelper.PrepareCustomerRewardPointsAsync(customerRewardPointsModel);

                return SuccessResponse(await _localizationService.GetResourceAsync("Nop.Api.RewardPointsList"), model);
            }
            catch (Exception ex)
            {
                return ErrorResponse(ex.Message, HttpStatusCode.BadRequest);
        ...
        #region Reorder
        [HttpPost]
        [Route("/api/ReOrder", Name = "ApiReOrder")]
        [ProducesResponseType(typeof(OrdersRootObject), (int)HttpStatusCode.OK)]
        [ProducesResponseType(typeof(string), (int)HttpStatusCode.Unauthorized)]
        [ProducesResponseType(typeof(ErrorsRootObject), (int)HttpStatusCode.BadRequest)]
        [ProducesResponseType(typeof(string), (int)HttpStatusCode.NotFound)]
        [ProducesResponseType(typeof(ErrorsRootObject), 422)]
        public virtual async Task<BaseResponseModel> ReOrder([FromBody] ReOrderRequestDto model)
        {
            var order = await _orderService.GetOrderByIdAsync(model.OrderId);
            if (order == null)
            {
                return ErrorResponse(await _localizationService.GetResourceAsync("Order Not Found", 1), HttpStatusCode.NotFound);
            }
            if (model.UserId <= 0)
            {
            ...
            if (customer == null)
            {
                return ErrorResponse(await _localizationService.GetResourceAsync("Customer Not Found", 1), HttpStatusCode.NotFound);
            }

            await _orderProcessingService.ReOrderAsync(order);

            return SuccessResponse(await _localizationService.GetResourceAsync("Nop.Api.Order.Reorder.Placed.Success"));
        }
            ...
            string shippingRateComputationMethodSystemName, string shippingOptionName, int storeId, Customer customer, List<ShoppingCartItem> shoppingCartItems)
        {
            ...
            {
                var shippingOptionResponse = await _shippingService.GetShippingOptionsAsync(shoppingCartItems, await _customerService.GetCustomerShippingAddressAsync(customer), customer,
