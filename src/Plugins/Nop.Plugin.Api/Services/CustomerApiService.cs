...
namespace Nop.Plugin.Api.Services
{
    public class CustomerApiService : ICustomerApiService
    {
        ...
        private static readonly string PROFILE_PICTURE = NopCustomerApiDefaults.ProfilePicUrlAttribute.ToLowerInvariant();
        private static readonly string Address_Type = NopCustomerDefaults.LastSearchedAddressType.ToLowerInvariant();
        ...
        public async Task<IList<CustomerDto>> GetCustomersDtosAsync(
            DateTime? createdAtMin = null, DateTime? createdAtMax = null, int limit = Constants.Configurations.DEFAULT_LIMIT,
        ...
        public async Task<IList<CustomerDto>> SearchAsync(
            string queryParams = "", string order = Constants.Configurations.DEFAULT_ORDER,
        ...
        public async Task<CustomerDto> GetCustomerByIdAsync(int id, bool showDeleted = false)
        {
            ...
            if (customerAttributeMappings.Count > 0)
            {
                ...
                foreach (var mapping in customerAttributeMappings)
                {
                    ...
                    if (mapping.Attribute != null)
                    {
                        ...
                        else if (mapping.Attribute.Key.Equals(PROFILE_PICTURE, StringComparison.InvariantCultureIgnoreCase))
                        {
                            customerDto.ProfilePictureUrl = mapping.Attribute.Value;
                        }
                        else if (mapping.Attribute.Key.Equals(Address_Type, StringComparison.InvariantCultureIgnoreCase))
                        {
                            customerDto.AddressType = mapping.Attribute.Value;
                        }
                    }
                }
            }
            ...
            if (customer is not null)
                customerDto.IsGuest = await _customerService.IsGuestAsync(customer);
        ...
        private async Task<IList<CustomerDto>> HandleCustomerGenericAttributesAsync(
            ...
            string order = Constants.Configurations.DEFAULT_ORDER)
        {
        ...
        /// </returns>
        public virtual async Task<IPagedList<Address>> GetAddressesByCustomerIdAsync(
          int customerId,
          int? pageIndex = 0,
          ...
          int storeId = 0)
        {
            ...
            var pagedAddresses = new PagedList<Address>(query.ToList(), page, pageSize ?? Constants.Configurations.DEFAULT_LIMIT);
