...
namespace Nop.Plugin.Api.Services
{
    public class OrderApiService : IOrderApiService
    {
        ...
        public IList<Order> GetOrdersByCustomerId(int customerId)
        {
            ...
            return new ApiList<Order>(query, 0, Constants.Configurations.MAX_LIMIT);
        }

        public async Task<IPagedList<Order>> GetOrders(
            ...
            int? storeId = null)
        {
            var query = await (GetOrdersQuery(createdAtMin, createdAtMax, status, paymentStatus, shippingStatus, ids, customerId, storeId)).ToListAsync();

            if (sinceId > 0)
                query = query.Where(order => order.Id > sinceId).ToList();

            return new PagedList<Order>(query.ToList(), page - 1, limit);
        }
