...
namespace Nop.Plugin.Api.Services
{
    public class CategoryApiService : ICategoryApiService
    {
        ...
        public virtual async Task<IPagedList<Category>> GetCategories(
            IList<int> ids = null,
        ...
        {
            var query = await (GetCategoriesQuery(createdAtMin, createdAtMax, updatedAtMin, updatedAtMax, publishedStatus, productId, ids, parentCategoryId))
                //store mapping
                .WhereAwait(async c => await _storeMappingService.AuthorizeAsync(c)).ToListAsync();

            if (sinceId > 0)
                query = query.Where(c => c.Id > sinceId).ToList();

            //return new ApiList<Category>(query, page - 1, limit);
            return new PagedList<Category>(query.ToList(), page - 1, limit);
        }
        ...
        {
            var query = _categoryRepository.Table.Where(c => c.ParentCategoryId == 0);
