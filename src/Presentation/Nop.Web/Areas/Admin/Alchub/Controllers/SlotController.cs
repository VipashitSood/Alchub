...
{
    public partial class SlotController : BaseAdminController
    {
        #region Fields
        ...
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageSlots))
                return AccessDeniedView();


            //prepare model
            var model = await _slotModelFactory.PrepareZoneSearchModel(new SlotSearchModel());

            return View(model);
        }

        [HttpPost]
        public async Task<IActionResult> ZoneList(SlotSearchModel searchModel)
        {
            if (!await _permissionService.AuthorizeAsync(StandardPermissionProvider.ManageSlots))
                return AccessDeniedView();
            ...
            var model = await _slotModelFactory.PrepareZoneListModel(searchModel);
        ...
        public async Task<IActionResult> Create()
        {
            ...
            //prepare model
            var model = await _slotModelFactory.PrepareZoneModel(new ZoneModel(), null);
        ...
        public async Task<IActionResult> Create(ZoneModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                if (model != null)
                {
                    ...
                    if (vendor != null)
                    {
                        model.SelectedVendorId = vendor.Id;
                        model.CreatedBy = 0;
                    }
                    else
                    {
                    ...
                    var existingVendor = await _slotService.GetVendorZones(true, vendor != null ? vendor.Id : model.SelectedVendorId, false, model.CreatedBy);
                    if (existingVendor != null)
                    {
                        _notificationService.ErrorNotification(await _localizationService.GetResourceAsync("Alchub.Already.Created.For.Vendor"));
                        return RedirectToAction("List");
                    }
                    ...
                    else
                    {
                        ...
                        zone.VendorId = vendor != null ? vendor.Id : model.SelectedVendorId;
                        if (vendor != null)
                        {
                            zone.CreatedBy = 0;
                        }
                ...
                {
                    model = await _slotModelFactory.PrepareZoneModel(model, null);
        ...
        #region Free Slots Availability
        ...
        public async Task<JsonResult> GetAvailableCalendarEvents(string start, string end, int zoneId)
        {
            try
            {
                ...
                var events = await _slotService.GetSlotByZoneAndDate(startDate, endDate, zoneId);
                if (events.ToList() != null)
                {
                    return Json(events.Select(x => new CalendarEventDto(
                                x.Id,
                                x.Start,
                                (x.End.TimeOfDay.Hours == 23 && x.End.TimeOfDay.Minutes == 59) ? x.End.AddMinutes(1) : x.End,//handel 12am display purpose
                                "",
        ...
        public async Task<ActionResult> AddAvailableCalendarEvents(string start, string end, int id, int zoneId, bool isrecurring, RecurringType recurringtype, string weekdays, string sequenceIds, int capacity, decimal price, bool isunavailable, string name) /*AddAvailableCalendarEvents start ,end, id, sectorid, upcomingweek ,sequenceid */
        {
            try
            {
                ...
                var eventDto = new SlotModel
                {
                    Start = startDate,
                    End = (endDate.TimeOfDay.Hours == 23 && endDate.TimeOfDay.Minutes == 59) ? endDate.AddMinutes(1) : endDate,//handle 12am, display purpose
                ...
                if (id > 0)
                {
                    eventDto.SelectedCategoryIds = (await _slotService.GetSlotCategoriesBySlotIdAsync(id))
        ...
        [HttpPost]
        public async Task<ActionResult> AddAvailableCalendarEvents(SlotModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    var sameSlotCount = await _slotService.GetSameSlotwithSameStartEnd(model.Start, model.End, model.Zone.Id);
                ...
                {
                    var zonecount = await _slotService.GetUpcomingWeekStartEnd(model.Start, model.End, model.EndDate, model.Zone.Id, model.RecurringType, model.WeekDays);
                ...
                try
                {
                    ...
                    if (model.IsRecurring == false)
                    {
                        ...
                        await _slotService.InsertZoneFreeSlotCalendarEvent(freeslot);
        ...
        public async Task<JsonResult> RemoveEvents(string data, string start, string end, string enddate, int zoneId, bool isrecurring, RecurringType recurringtype, string weekdays, int capacity, decimal price, string selectedcategories)
        {
            if (data != null)
            {
                ...
                foreach (var id in Ids)
                {
                    ...
                    await _slotService.DeleteSlotIdCategory(calendarId);
                    await _slotService.Remove(calendarId);
        ...
        [HttpPost]
        public async Task<JsonResult> RemoveAvailableCalendarEvents(SlotModel model)
        {
            ...
            {
                // Remove slot
                await _slotService.DeleteSlotIdCategory(model.Id);
                await _slotService.Remove(model.Id);
        ...
        [HttpPost]
        public async Task<JsonResult> RemoveAllAvailableCalendarEvents(SlotModel model)
        {
            ...
            var sectorFreeSlotAllowedCalendarEventList = await _slotService.GetSequenceId(model.Sequence_Id);
            foreach (var slot in sectorFreeSlotAllowedCalendarEventList)
            {

                await _slotService.DeleteSlotIdCategory(slot.Id);
                await _slotService.Delete(slot);
        ...
        [HttpPost]
        public async Task<JsonResult> UpdateAvailableCalendarEvents(SlotModel model)
        {
            ...
            try
            {
                ...
                var zoneslot = await _slotService.GetSlotById(model.Id);
                if (zoneslot != null)
                {
                    ...
                    await _slotService.Update(zoneslot);
        ...
        #region Slot category Mapping 
        protected virtual async Task SaveCategoryMappingsAsync(Slot slot, SlotModel model)
        {
            var existingProductCategories = await _slotService.GetSlotCategoriesBySlotIdAsync(slot.Id);

            //delete categories

            foreach (var existingProductCategory in existingProductCategories)
                if (!model.SelectedCategoryIds.Contains(existingProductCategory.CategoryId))
                    await _slotService.DeleteSlotCategory(existingProductCategory);
            ...
            if (model.SelectedCategoryIds != null)
            {
                ...
                {
                    if (_slotService.FindSlotCategory(existingProductCategories, slot.Id, categoryId) == null)
                    {
                        //find next display order
                        var existingCategoryMapping = await _slotService.GetSlotCategoriesByCategoryIdAsync(slot.Id, categoryId, showHidden: true);
                        if (existingCategoryMapping.Any())
                            continue;

                        await _slotService.InsertSlotCategory(new SlotCategory
                        {
                            SlotId = slot.Id,
        ...
        protected virtual async Task UpdateCategoryMappingsAsync(Slot slot, SlotModel model)
        {
            var existingProductCategories = await _slotService.GetSlotCategoriesBySlotIdAsync(slot.Id);
            ...
            foreach (var existingProductCategory in existingProductCategories)
                ...
                {
                    await _slotService.DeleteSlotCategory(existingProductCategory);
                    await DeleteProductSlotExcludeCategoriesAsync(slot);
                }
        ...
        protected virtual async Task DeleteProductSlotExcludeCategoriesAsync(Slot slot)
        {
