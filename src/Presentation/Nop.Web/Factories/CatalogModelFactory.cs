...
namespace Nop.Web.Factories
{
  ...
  //      /// <summary>
		...
		//public virtual async Task PrepareFilterSubCategory(CatalogProductsCommand pagingFilteringModel, CatalogProductsCommand command)
  //      {
  //          var categoryName = _settingService.GetSettingByKey<string>("SOS.Filter.Categories", "");
  //          if (!string.IsNullOrEmpty(categoryName))
  //          {
  //              var category = (await _categoryService.GetAllCategoriesAsync()).Where(x=>x.Name);
  //              //subcategories
  //              var subCategories = _categoryService.GetAllCategoriesByParentCategoryId(category.Id);

  //              string subCategoryIds = pagingFilteringModel.SearchTermFilter.GetSubCategoryId(_webHelper);
  //              int subcategory = 0;
  //              if (!string.IsNullOrEmpty(subCategoryIds))
  //              {
  //                  subcategory = Convert.ToInt32(subCategoryIds);
  //              }
  //              //prepare available model sorting options
  //              var currentPageUrl = _webHelper.GetThisPageUrl(true);
  //              pagingFilteringModel.AvailableCategories.Add(new SelectListItem
  //              {
  //                  Value = "0",
  ...
  //                      Selected = option.Id == subcategory
  //                  });
  //              }
  //          }
  //      }
