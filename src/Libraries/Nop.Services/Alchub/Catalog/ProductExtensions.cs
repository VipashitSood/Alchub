...
namespace Nop.Services.Catalog
{
    public static class ProductExtensions
    {
        ...
        /// </remarks>
        public static IQueryable<Product> OrderBy(this IQueryable<Product> productsQuery, IRepository<LocalizedProperty> localizedPropertyRepository, Language currentLanguage, ProductSortingEnum orderBy, IQueryable<Product> subProductsQuery = null)
        {
            ...
            switch (orderBy)
            {
                ...
                case ProductSortingEnum.PriceAsc:
                    var productsQueryAsc =
                            from p in productsQuery
                            let price = p.ProductTypeId == (int)ProductType.SimpleProduct ? subProductsQuery.Where(x => x.UPCCode == p.UPCCode).OrderBy(x => x.Price)
                                                                                                            .FirstOrDefault().Price :
                                                                                            subProductsQuery.Where(x => x.UPCCode == tempMasterProductQuery.OrderBy(x => x.DisplayOrder).FirstOrDefault(y => y.ParentGroupedProductId == p.Id).UPCCode)
                                                                                                            .OrderBy(z => z.Price).FirstOrDefault().Price
                            orderby price
                            select p;
                    return productsQueryAsc;

                //PriceDesc
                case ProductSortingEnum.PriceDesc:
                    var productsQueryDesc =
                          from p in productsQuery
                          let price = p.ProductTypeId == (int)ProductType.SimpleProduct ? subProductsQuery.Where(x => x.UPCCode == p.UPCCode).OrderBy(x => x.Price)
                                                                                                          .FirstOrDefault().Price :
                                                                                          subProductsQuery.Where(x => x.UPCCode == tempMasterProductQuery.OrderBy(x => x.DisplayOrder).FirstOrDefault(y => y.ParentGroupedProductId == p.Id).UPCCode)
                          ...
                          select p;
                    return productsQueryDesc;

                //CreatedOn
                case ProductSortingEnum.CreatedOn:
                    return productsQuery.OrderByDescending(p => p.CreatedOnUtc);

                //Position
                case ProductSortingEnum.Position when productsQuery is IOrderedQueryable:
                    return productsQuery.OrderBy(p => p.DisplayOrder).ThenBy(p => p.Id);
